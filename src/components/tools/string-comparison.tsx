...
export function StringComparison() {
  ...
  const handleCompare = () => {
    ...
    if (leftText === rightText) {
      ...
      setRightDiff(rightText.split("").map(c => ({ char: c, type: "match" })))
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="left-input" className="text-zinc-300">Original Text (Left)</Label>
          <textarea
            id="left-input"
            value={leftText}
            onChange={(e) => {
        ...
        <div className="space-y-2">
          <Label htmlFor="right-input" className="text-zinc-300">Modified Text (Right)</Label>
          <textarea
            id="right-input"
            value={rightText}
            onChange={(e) => {
        ...
        <div className="space-y-6 pt-2">
          ...
          {discrepancyCount === 0 ? (
            ...
            <div className="flex items-center space-x-2 p-3 border border-red-800/50 bg-red-950/20 text-red-400 text-sm rounded-md">
              ...
              <span>
                Discrepancy found: <strong>{discrepancyCount}</strong> difference{discrepancyCount > 1 ? "s" : ""} detected.
              </span>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label className="text-zinc-400 text-xs">Diff Highlights (Left)</Label>
              <div className="w-full min-h-32 rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm font-mono whitespace-pre-wrap break-all overflow-y-auto">
                {leftDiff.map((item, idx) => (
                  <span
                    ...
                    className={
                      item.type === "removed"
                        ? "bg-red-950 text-red-300 border border-red-900/50 px-0.5 rounded-[2px] font-bold"
                        : "text-zinc-100"
                    }
                  >
                    {item.char}
            ...
            <div className="space-y-2">
              <Label className="text-zinc-400 text-xs">Diff Highlights (Right)</Label>
              <div className="w-full min-h-32 rounded-md border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm font-mono whitespace-pre-wrap break-all overflow-y-auto">
                {rightDiff.map((item, idx) => (
                  <span
                    ...
                    className={
                      item.type === "added"
                        ? "bg-green-950 text-green-300 border border-green-900/50 px-0.5 rounded-[2px] font-bold"
                        : "text-zinc-100"
                    }
                  >
                    {item.char}
